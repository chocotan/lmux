...
[[ -x "${LMUX_TEST_SHELL:-/usr/bin/zsh}" ]] || { echo "missing test shell: ${LMUX_TEST_SHELL:-/usr/bin/zsh}" >&2; exit 1; }

cargo build -p lmux-app >/dev/null
TMP="$(mktemp -d)"
ORIGINAL_FCITX_STATE="$(fcitx5-remote 2>/dev/null || echo 1)"
cleanup() {
  local status=$?
  if [[ "$status" -ne 0 && -f "$TMP/lmux.log" ]]; then
    echo "--- lmux.log ---" >&2
    tail -80 "$TMP/lmux.log" >&2
  fi
  [[ -n "${PID:-}" ]] && kill -9 "$PID" 2>/dev/null || true
  if [[ -f "$XDG_DATA_HOME/lmux/state.json" ]]; then
    python3 - "$XDG_DATA_HOME/lmux/state.json" <<'PY' | while read -r session; do
import json,sys
...
[[ -n "${WID:-}" ]] || { cat "$TMP/lmux.log" >&2; exit 1; }
...
sleep .2
xdotool type --delay 2 "echo LMUXASCII"
xdotool key Return
...
COLOR_TMUX="$(python3 - "$XDG_DATA_HOME/lmux/state.json" <<'PY'
import json,sys
print(json.load(open(sys.argv[1]))['sessions'][0]['tmux_session'])
PY
)"
tmux -L lmux send-keys -t "$COLOR_TMUX" -l -- "printf '\\033[31mLMUX_RED\\033[0m\\n'"
tmux -L lmux send-keys -t "$COLOR_TMUX" Enter
sleep .5
import -window "$WID" "$ARTIFACTS/01-color-cursor-input.png"

# 协议回放：输入真的进 PTY。截图像素：红色文字和蓝色 cursor 真的被 GPUI 画出。
LMUX_SOCKET="$XDG_DATA_HOME/lmux/lmux.sock" SCREENSHOT="$ARTIFACTS/01-color-cursor-input.png" python3 - <<'PY'
import os, socket, json, base64
from PIL import Image
p=os.environ['LMUX_SOCKET']
...
assert 'LMUXASCII' in text, text[-500:]
assert 'LMUX_RED' in text, text[-500:]
...
read -r HOOK_AGENT HOOK_TMUX < <(LMUX_SOCKET="$XDG_DATA_HOME/lmux/lmux.sock" python3 - <<'PY'
...
HOOK_TOKEN="$(tmux -L lmux show-environment -t "$HOOK_TMUX" LMUX_HOOK_TOKEN | cut -d= -f2-)"
run_hook() {
  local payload="${2:-}"
  printf '%s' "$payload" | LMUX_SOCKET="$XDG_DATA_HOME/lmux/lmux.sock" LMUX_AGENT_ID="$HOOK_AGENT" \
    LMUX_HOOK_TOKEN="$HOOK_TOKEN" node "$XDG_DATA_HOME/lmux/hooks/report.mjs" "$1"
}
...
xdotool key ctrl+w
sleep .2
STATE="$XDG_DATA_HOME/lmux/state.json" LMUX_SOCKET="$XDG_DATA_HOME/lmux/lmux.sock" python3 - <<'PY'
