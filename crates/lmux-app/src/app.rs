...
fn format_relative_time(then: u64) -> String {
...
fn panel_icon(data: &[u8], color: u32) -> gpui::Div {
    let glyph = if data == SPLIT_HORIZONTAL_ICON {
        "↔"
    } else if data == SPLIT_VERTICAL_ICON {
        "↕"
    } else if data == MAXIMIZE_ICON {
        "⛶"
    } else if data == RESTORE_ICON {
        "❐"
    } else {
        "×"
    };
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .bg(rgba(0xffffffff))
                    .border_1()
                    .border_color(rgba(0x3d6cd8ff))
                    .text_size(px(11.))
                    .text_color(rgba(0x242831ff))
...
enum NewSessionTarget {
    Local(lmux_core::model::ProjectId),
    Remote {
        host: String,
...
pub struct LmuxApp {
    ...
    /// 远端安装/升级进度（host → 进度）
    bootstrap_progress: HashMap<String, lmux_client::BootstrapProgress>,
    /// 远程操作失败的短暂提示
...
pub struct ToastNotification {
    ...
    pub title: String,
    pub message: String,
    pub status: lmux_core::model::AgentStatus,
...
enum PaletteItem {
    AgentJump {
        ...
        project_name: String,
        title: String,
        agent_type: lmux_core::model::AgentType,
        status: lmux_core::model::AgentStatus,
    },
    Preset {
...
impl LmuxApp {
    pub fn new(
        ...
        for saved in &persisted.remote_configs {
            ...
            let auth = match &saved.auth {
                ...
                lmux_store::PersistedRemoteAuth::Password { username, password } => {
                    ...
                    this.update(cx, |this, cx| {
                        match ev {
                            lmux_client::ClientEvent::StateChanged { host, state } => {
                                ...
                                // 到达稳态后清除进度显示
                                if !matches!(state, lmux_client::RemoteState::Connecting(_)) {
                                    this.bootstrap_progress.remove(&host);
                                }
                                this.remote_states.insert(host, state);
                            }
                            lmux_client::ClientEvent::BootstrapProgress { host, progress } => {
                            ...
                            lmux_client::ClientEvent::StatusChanged {
                                ...
                                if let Some(snap) = this.remote_snaps.get_mut(&host) {
                                    ...
                                    {
                                        a.status = to;
                                        if to == lmux_core::model::AgentStatus::Done {
                                            a.seen = false;
                                        }
                                    }
                                }
        ...
        let is_dark = persisted.dark_mode.unwrap_or(false);
        ...
        for agent in opened {
            ...
            if let Some(session) = session {
                let term = Self::create_local_term(agent.clone(), session, cx);
    ...
    fn persist(&self) {
        ...
        let remote_configs: Vec<lmux_store::PersistedRemote> = self
            .remotes
            .iter()
            .map(|host| {
                ...
                let auth = match &host.cfg.auth {
                    ...
                    lmux_client::SshAuth::Password { username, password } => {
                        lmux_store::PersistedRemoteAuth::Password {
                            username: username.clone(),
                ...
                lmux_store::PersistedRemote { target, auth }
            })
            .collect();
        let app = lmux_store::PersistedApp {
            ...
            maximized_pane: None,
            window: None,
            dark_mode: Some(self.theme_mode == ThemeMode::Dark),
        };
    ...
    fn find_agent(&self, agent: &AgentId) -> Option<lmux_core::model::AgentInstance> {
    ...
    fn push_notification(
        ...
        let now_secs = lmux_core::model::now_secs();
    ...
    fn activate_tab(&mut self, pane: &PaneId, agent: &AgentId) {
        ...
        if let Some(a) = self.last_snapshot.agent_mut(agent) {
            a.seen = true;
            if a.status == lmux_core::model::AgentStatus::Done {
                a.status = lmux_core::model::AgentStatus::Idle;
            }
        }
    ...
    fn create_remote_term(
        ...
        let term = cx.new(|cx| TermView::new_remote(agent.clone(), vterm, remote_input, cx));
        cx.subscribe(
            &term,
            |this, _term, ev: &crate::term_view::TermEnterEvent, cx| {
                this.mark_agent_working(&ev.0, cx);
    ...
    fn open_agent(&mut self, agent: &AgentId, cx: &mut Context<Self>) {
        ...
        if !self.terms.contains_key(agent) {
            ...
            if let Some(sess) = sess {
                let term = Self::create_local_term(agent.clone(), sess, cx);
    ...
    fn open_remote_agent(&mut self, agent: &AgentId, cx: &mut Context<Self>) {
        ...
        if !self.terms.contains_key(agent) {
            ...
            let term = Self::create_remote_term(agent.clone(), vterm.clone(), command_tx, cx);
            self.terms.insert(agent.clone(), term);
    ...
    fn spawn_preset(
        ...
        self.server.dirty.bump();
        let term = Self::create_local_term(agent_id.clone(), session, cx);
    ...
    fn spawn_remote_agent(
        ...
        cx.spawn_in(window, async move |this, cx| {
            let result = runtime
                .spawn(async move {
                    lmux_client::spawn_agent(&endpoint, &project, preset.as_ref()).await
                })
                .await;
            let _ = this.update_in(cx, |this, window, cx| match result {
                ...
                Ok(Err(error)) => {
                    ...
                    // 类型不匹配通常意味着远端仍在运行旧版 lmux，
                    // 直接切换到已有的更新引导状态。
                    if text.contains("远端 lmux 版本过旧") {
    ...
    fn compute_palette_items(&self, cx: &Context<Self>) -> Vec<PaletteItem> {
        ...
        if let Some(target) = &self.new_session_target {
            ...
            // 全局命令面板 (Ctrl+K)：
            // 1. 本地与远程所有 Agent（会话快速跳转）
            for project in &self.last_snapshot.projects {
                for agent in self.last_snapshot.agents_of(&project.id) {
                    items.push(PaletteItem::AgentJump {
            ...
            for (host_name, snap) in &self.remote_snaps {
                for project in &snap.projects {
                    for agent in snap.agents_of(&project.id) {
                        items.push(PaletteItem::AgentJump {
                            agent_id: agent.id.clone(),
                            machine_name: host_name.clone(),
                            project_name: project.name.clone(),
                            title: agent.title.clone(),
                            agent_type: agent.agent_type,
                            status: agent.status,
            ...
            items.push(PaletteItem::Action {
                id: "cmd-split-h",
                label: "水平分屏 (Horizontal Split)",
                shortcut: Some("h"),
                icon: "◫",
            });
            items.push(PaletteItem::Action {
                id: "cmd-split-v",
                label: "垂直分屏 (Vertical Split)",
                shortcut: Some("v"),
                icon: "⊟",
            });
            items.push(PaletteItem::Action {
            ...
            items.push(PaletteItem::Action {
                ...
                label: if self.theme_mode == ThemeMode::Dark {
                    "切换为纯净浅色模式 (Light Mode)"
                } else {
                    "切换为极简深色模式 (Dark Mode)"
                },
                shortcut: None,
                icon: if self.theme_mode == ThemeMode::Dark {
                    "☀️"
                } else {
                    "🌙"
                },
            ...
            items
                ...
                .filter(|item| match item {
                    PaletteItem::AgentJump {
                        ...
                        let text = format!(
                            "{machine_name} {project_name} {title} {}",
                            agent_type.as_str()
                        )
                        .to_lowercase();
                        text.contains(&query)
    ...
    fn execute_palette_item(
        ...
        match item {
            PaletteItem::AgentJump { agent_id, .. } => {
                self.new_session_target = None;
                self.jump_to_agent(&agent_id, window, cx);
            }
            PaletteItem::Preset { preset } => {
            ...
            for (index, item) in items.into_iter().enumerate() {
                ...
                let row = match item {
                    PaletteItem::AgentJump {
                        ...
                        div()
                            .id(gpui::ElementId::Name(format!("pal-agent-{index}").into()))
                            .flex()
                            .items_center()
                            .gap_2()
                            .px_3()
                            ...
                            .child(div().w(px(7.)).h(px(7.)).rounded_full().bg(rgba(dot_color)))
                            .child(
                                div()
                                    .font_weight(gpui::FontWeight::MEDIUM)
                                    .child(format!("{machine_name} · {project_name}")),
                            )
                            .child(
                                div()
                                    .text_color(rgba(theme.fg1))
                                    .child(truncate(&title, 24)),
                            )
                            .child(
                                div()
                                    .ml_auto()
                    ...
                    PaletteItem::Action {
                        ...
                        .child(div().w(px(16.)).text_color(rgba(theme.accent)).child(icon))
                        .child(label)
                            ...
                            div()
                                ...
                                .when(axis == SplitAxis::Horizontal, |el| {
                                    el.w(px(14.))
                                        ...
                                        .ml(px(-6.))
                                        .mr(px(-6.))
                                        .child(div().w(px(2.)).h_full().bg(rgba(theme.line)))
                                })
                                .when(axis == SplitAxis::Vertical, |el| {
                                    el.h(px(14.))
                                        ...
                                        .mt(px(-6.))
                                        .mb(px(-6.))
                                        .child(div().h(px(2.)).w_full().bg(rgba(theme.line)))
                                })
                                .hover(|s| s.bg(rgba(theme.bg2)))
                                .on_click(cx.listener({
            ...
            PaneNode::Leaf { group } => {
                ...
                for tab_id in group.tabs.clone() {
                    ...
                    let status = agent_opt
                        .as_ref()
                        .map(|a| a.status)
                        .unwrap_or(lmux_core::model::AgentStatus::Idle);
                    ...
                    let tab = div()
                        ...
                        .when(!att.is_alerting && is_active, |el| el.bg(rgba(theme.bg0)))
                        .border_r_1()
                        .border_color(rgba(theme.line))
                        .hover(|s| s.bg(rgba(theme.bg2)))
                        .on_click(cx.listener({
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
                        ...
                        let row = div()
                            ...
                            .child(
                                div()
                                    ...
                                    .px_1()
                                    .rounded_xs()
                                    .bg(rgba(if active { theme.bg3 } else { theme.bg2 }))
                                    .text_color(rgba(theme.fg2))
        ...
        for host in &self.remotes {
            ...
            let (dot_color, status_text, remediation) = match self.remote_states.get(&name) {
                ...
                Some(lmux_client::RemoteState::AuthenticationFailed(_)) => {
                    (theme.red, "认证失败", None)
                }
                Some(lmux_client::RemoteState::Connecting(stage)) => {
                    (theme.yellow, stage.label(), None)
                }
                Some(lmux_client::RemoteState::Offline(_)) => (theme.fg1, "离线", None),
            ...
            let status_text = if matches!(
                ...
                self.remote_states.get(&name),
                Some(lmux_client::RemoteState::Offline(_))
            ) {
                "已断开".into()
            } else {
            ...
            let machine_attention = snap_ref
                .map(|snapshot| {
                    if snapshot
                        .agents
                        .iter()
                        .any(|agent| agent.status == lmux_core::model::AgentStatus::Blocked)
                    {
                            ...
                            .on_click(cx.listener({
                                ...
                                move |this, _event, window, cx| {
                                    if is_auth_failed {
                                        ...
                                        match &host_cfg.auth {
                                            lmux_client::SshAuth::Password { username, .. } => {
                                                this.connect_auth_mode = ConnectAuthMode::Password;
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .text_size(px(10.))
                            .text_color(rgba(theme.fg2))
                            .hover(|s| s.text_color(rgba(theme.accent)))
                            .on_click(cx.listener(|this, _ev, _window, cx| {
            ...
            .child(
                div()
                    ...
                    .children(self.notifications.iter().enumerate().map(|(idx, n)| {
                        ...
                        let time_str = format_relative_time(n.time_secs);
                        div()
                            .id(gpui::ElementId::Name(format!("notif-item-{idx}").into()))
                            .flex()
                            .flex_col()
                            .gap_0p5()
                            .px_3()
                            ...
                            .border_color(rgba(theme.line))
                            .when(is_unread, |el| {
                                el.bg(rgba(if theme.mode == ThemeMode::Dark {
                                    0x2c313aff
                                } else {
                                    0xf0eee6ff
                                }))
                                ...
                                div()
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .child(format!(
                                                "{} · {} · {}",
                                                n.machine_name, n.project_name, n.agent_type
                                            )),
                                    )
                                    .child(
                                        div()
                            ...
                            .when_some(n.message.clone(), |el, msg| {
                                el.child(
                                    div()
                                        .text_size(px(11.))
            ...
            let toast_container = div()
                ...
                .children(self.toasts.iter().map(|t| {
                    let dot_color = match t.status {
                        ...
                        lmux_core::model::AgentStatus::Done => theme.green,
                        lmux_core::model::AgentStatus::Working => theme.accent,
                        lmux_core::model::AgentStatus::Idle => theme.fg2,
                    };
        ...
        if let Some((message, _)) = self.error_toast.clone() {
            root = root.child(
                div()
                    ...
                    .w(px(420.))
                    .p_3()
                    .bg(rgba(if theme.mode == ThemeMode::Dark {
                        0x542b31ff
                    } else {
                        0xffe5e5ff
                    }))
...
fn compute_attention_style(
    ...
    match status {
        ...
        lmux_core::model::AgentStatus::Done if !seen => {
...
fn render_status_indicator(
    ...
    match status {
        lmux_core::model::AgentStatus::Working => render_pi_loading_spinner(spinner_frame, theme),
        lmux_core::model::AgentStatus::Blocked => container.child(
            div()
                .w(px(6.))
                .h(px(6.))
                .rounded_full()
        ...
        lmux_core::model::AgentStatus::Done if is_error => {
            container.child(div().w(px(6.)).h(px(6.)).rounded_full().bg(rgba(theme.red)))
        }
        lmux_core::model::AgentStatus::Done => container.child(
            div()
                .w(px(6.))
                .h(px(6.))
                .rounded_full()
        ...
        lmux_core::model::AgentStatus::Idle => {
...
fn format_upload_phase(progress: &lmux_client::BootstrapProgress) -> String {
...
mod tests {
    ...
    fn upload_progress_text_shows_bytes_and_percent() {
        use lmux_client::BootstrapPhase;
        let progress = lmux_client::BootstrapProgress {
        ...
        // 无字节数时退化为纯百分比
        let text = format_upload_phase(&lmux_client::BootstrapProgress {
