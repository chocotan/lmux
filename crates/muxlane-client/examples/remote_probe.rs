use muxlane_client::{HostCfg, RemoteHost, RemoteState, SshAuth, Target};
use tokio::sync::mpsc;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let host = std::env::var("MUXLANE_TEST_HOST")?;
    let username = std::env::var("MUXLANE_TEST_USERNAME")?;
    let password = std::env::var("MUXLANE_TEST_PASSWORD")?;
    ...
    for _ in 0..120 {
        match remote.state().await {
            ...
            RemoteState::NeedsInstall { .. } => anyhow::bail!("remote Muxlane needs installation"),
            RemoteState::NeedsStart { .. } => anyhow::bail!("remote Muxlane needs start"),
            RemoteState::NeedsUpgrade { .. } => {
                if std::env::var("MUXLANE_TEST_UPGRADE").as_deref() == Ok("1") && !upgraded {
                    std::sync::Arc::clone(&remote).upgrade_and_retry().await?;
                    upgraded = true;
                } else if !upgraded {
                    anyhow::bail!("remote Muxlane needs upgrade");
                }
            }
