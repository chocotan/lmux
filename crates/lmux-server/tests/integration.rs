...
use lmux_core::protocol::{methods, read_frame, write_frame, EventMsg, Request, Response};
...
async fn spawn_server() -> (
    ...
    let sock = dir.path().join("lmux.sock");
    ...
    let server = LmuxServer::new(sock.clone(), Arc::clone(&state), dirty.clone());
...
async fn launch_agent(
    ...
    let sess = lmux_term::PtySession::spawn(cfg).unwrap();
...
async fn remote_term_input_reaches_pty_and_project_add_validates_path() {
    ...
    write_frame(
        ...
        &Request {
            ...
            params: serde_json::to_value(lmux_core::protocol::TermInputParams {
                agent: agent.clone(),
                data_b64: lmux_term::b64_encode(b"hello\n"),
            })
            .unwrap(),
        },
    )
...
async fn project_delete_destroys_scoped_sessions_and_state() {
    ...
    let result: lmux_core::protocol::DeleteScopeResult =
...
async fn state_list_returns_snapshot() {
    ...
    assert_eq!(resp.id, 1);
    let snap: lmux_core::model::Snapshot = serde_json::from_value(resp.result.unwrap()).unwrap();
...
async fn term_subscribe_replay_and_incremental() {
    ...
    .unwrap();
    let resp: Response = serde_json::from_value(read_frame(&mut conn).await.unwrap()).unwrap();
    let result: lmux_core::protocol::TermSubscribeResult =
        serde_json::from_value(resp.result.unwrap()).unwrap();
    let replay = lmux_term::b64_decode(&result.replay_b64).unwrap();
    assert!(
        ...
        match frame {
            Ok(Ok(v)) => {
                if v["event"] == "term.data" {
                    let data =
                        lmux_term::b64_decode(v["params"]["data_b64"].as_str().unwrap()).unwrap();
...
async fn node_hook_script_reports_with_env_identity() {
    ...
    std::fs::write(&script, lmux_core::hook::REPORT_SCRIPT).unwrap();
    let token = srv.hook_token(&agent);
    let mut child = tokio::process::Command::new("node")
        .arg(&script)
        .arg("done")
        .env("LMUX_SOCKET", &sock)
        .env("LMUX_AGENT_ID", &agent)
        .env("LMUX_HOOK_TOKEN", token)
...
async fn second_server_cannot_unlink_active_socket() {
    ...
    let mk_state = || {
        Arc::new(RwLock::new(ServerState::new(MachineInfo {
            machine_id: lmux_core::model::new_id("m"),
            name: "box".into(),
            os: "linux".into(),
            version: "0.1".into(),
        })))
    };
    let first = LmuxServer::new(sock.clone(), mk_state(), DirtyFlag::new());
    ...
    let second = LmuxServer::new(sock.clone(), mk_state(), DirtyFlag::new());
...
async fn agent_delete_kills_and_cleans_state() {
    ...
    let ev = tokio::time::timeout(std::time::Duration::from_secs(1), read_frame(&mut events))
        ...
        .unwrap()
        .unwrap();
    assert_eq!(ev["event"], lmux_core::protocol::events::STATE_CHANGED);
}
