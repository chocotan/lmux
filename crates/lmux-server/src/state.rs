...
use lmux_core::detect::{DetectionEngine, HookEvent};
use lmux_core::model::{AgentId, AgentInstance, AgentStatus, MachineInfo, Project, Snapshot};
use lmux_core::protocol::{AgentReportParams, EventMsg};
...
pub struct ServerState {
    ...
    pub events: tokio::sync::broadcast::Sender<lmux_core::protocol::EventMsg>,
}
...
impl ServerState {
    pub fn new(machine: MachineInfo) -> Self {
        ...
        for manifest in lmux_core::detect::builtin_manifests() {
    ...
    pub fn add_agent(&mut self, project: Project, mut instance: AgentInstance) {
        ...
        instance.status_since = lmux_core::model::now_secs();
    ...
    pub fn remove_project(&mut self, project: &lmux_core::model::ProjectId) -> Vec<AgentId> {
    ...
    pub fn observe_screen(
        &mut self,
        agent: &AgentId,
        input: &lmux_core::detect::ScreenInput,
    ) -> Vec<EventMsg> {
    ...
    pub async fn report_hook(&mut self, params: &AgentReportParams) -> Vec<EventMsg> {
                ...
                .is_some_and(|message| !message.trim().is_empty())
        {
            let msg = EventMsg::new(
                ...
                serde_json::to_value(lmux_core::protocol::AgentStatusEvent {
    ...
    fn apply_status(
        ...
        inst.status = to;
        inst.status_since = lmux_core::model::now_secs();
...
mod tests {
    ...
    async fn same_status_hook_enriches_message_and_osc_updates_title() {
        ...
        st.add_agent(
            ...
            AgentInstance {
                id: agent.clone(),
                project: "p".into(),
                agent_type: lmux_core::model::AgentType::Pi,
                title: "Pi".into(),
                status: AgentStatus::Idle,
                status_since: 0,
                seen: true,
        ...
        let events = st
            ...
            .await;
        let payload: lmux_core::protocol::AgentStatusEvent =
        ...
        let title_events = st.observe_screen(
            ...
            &lmux_core::detect::ScreenInput {
                osc_title: Some("π - lmux repo".into()),
                ..Default::default()
            },
        );
    ...
    fn screen_detection_is_registered_and_done_seen_returns_idle() {
        ...
        st.agents.push(AgentInstance {
            id: agent.clone(),
            project: "p".into(),
            agent_type: lmux_core::model::AgentType::Claude,
            title: "Claude".into(),
        ...
        let input = lmux_core::detect::ScreenInput {
