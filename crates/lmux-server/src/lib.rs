//! lmux-server：本机 Unix socket 服务端（client 也能连它：对端机器的 lmux、或本机 hook 脚本）
mod state;
mod subs;
...
impl LmuxServer {
    ...
    pub fn hook_token(&self, agent: &lmux_core::model::AgentId) -> String {
    ...
    async fn persist_runtime_state(&self) -> anyhow::Result<()> {
        ...
        let mut persisted = lmux_store::load(&path).unwrap_or_default();
    ...
    pub async fn serve(self: Arc<Self>) -> anyhow::Result<()> {
        ...
        lock_file
            .try_lock_exclusive()
            .map_err(|_| anyhow::anyhow!("another lmux instance owns {}", lock_path.display()))?;
    ...
    async fn destroy_sessions(
        &self,
        agents: &[lmux_core::model::AgentId],
    ) -> (
        Vec<lmux_core::model::AgentId>,
        Vec<lmux_core::model::AgentId>,
    ) {
    ...
    async fn handle_conn(self: Arc<Self>, stream: UnixStream) -> anyhow::Result<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let eof = matches!(frame, Err(lmux_core::protocol::FrameError::Eof));
        ...
        loop {
            tokio::select! {
                frame = frame_rx.recv() => {
                    match frame {
                        Some(Ok(v)) => {
                            let req: lmux_core::protocol::Request = match serde_json::from_value(v) {
                                Ok(r) => r,
                                Err(e) => {
                            ...
                            match req.method.as_str() {
                                methods::SYSTEM_HELLO => {
                                    write_frame(
                                        ...
                                        &Response::ok(
                                            req.id,
                                            serde_json::to_value(lmux_core::protocol::HelloResult {
                                ...
                                methods::EVENTS_SUBSCRIBE => {
                                    ...
                                    let _ = ev_tx.send(EventMsg::new(lmux_core::protocol::events::STATE_CHANGED, serde_json::json!({}))).await;
                                }
                                        ...
                                        Ok(params) => {
                                            ...
                                            let prepared = {
                                                ...
                                                sess_opt.map(|sess| {
                                                    ...
                                                    (lmux_core::model::new_id("sub"), lmux_term::b64_encode(&snap), rx, sess)
                                                })
                                            };
                                            match prepared {
                                ...
                                methods::TERM_INPUT => {
                                    match serde_json::from_value::<lmux_core::protocol::TermInputParams>(req.params) {
                                        Ok(params) => {
                                ...
                                methods::TERM_RESIZE => {
                                    match serde_json::from_value::<lmux_core::protocol::TermResizeParams>(req.params) {
                                        Ok(params) => {
                                            let session = self.sessions.lock().await.get(&params.agent).cloned();
                                            match session {
                                                Some(session) => {
                                ...
                                methods::AGENT_SPAWN => {
                                    match serde_json::from_value::<lmux_core::protocol::AgentSpawnParams>(req.params) {
                                        Ok(params) => {
                                            ...
                                            match project {
                                                Some(project) => {
                                                    ...
                                                    match lmux_term::PtySession::spawn(cfg) {
                                                        Ok(session) => {
                                ...
                                methods::AGENT_DELETE => {
                                    match serde_json::from_value::<lmux_core::protocol::AgentDeleteParams>(req.params) {
                                        Ok(params) => {
                                ...
                                methods::PROJECT_ADD => {
                                    match serde_json::from_value::<lmux_core::protocol::ProjectAddParams>(req.params) {
                                        Ok(params) => {
                                ...
                                methods::PROJECT_DELETE => {
                                    match serde_json::from_value::<lmux_core::protocol::ProjectDeleteParams>(req.params) {
                                        Ok(params) => {
                                            let _lifecycle = self.lifecycle.lock().await;
                                            ...
                                            write_frame(&mut write_half, &Response::ok(
                                                req.id,
                                                serde_json::to_value(lmux_core::protocol::DeleteScopeResult {
                                                    destroyed_agents,
                                                    failed_agents,
                                                })?,
                                            )).await?;
                        ...
                        Some(Err(lmux_core::protocol::FrameError::Eof)) | None => break,
                ...
                Ok(()) = dirty_rx.changed() => {
                    ...
                    if cur != dirty_seen {
                        dirty_seen = cur;
                        let _ = ev_tx.try_send(EventMsg::new(lmux_core::protocol::events::STATE_CHANGED, serde_json::json!({})));
                    }
                }
                else => break,
            }
